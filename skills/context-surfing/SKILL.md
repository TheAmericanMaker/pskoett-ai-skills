...
3. The current session state from the Entire CLI
4. All project context files (see below)

The **wave anchor** is not held mentally. It is the intent-framed-agent output combined with the Entire CLI session state. Both are external, persistent artifacts. Every drift check reads from them directly — never from reconstructed memory.
...
3. **Track completions via the Entire CLI.** Log what's done, what's in progress, what's pending to the CLI session state as work progresses — not at exit. This feeds the handoff if needed and is the ground truth for the session.
4. **Resist scope creep.** If something interesting but out-of-scope appears, note it in the Entire CLI — don't chase it.
...
Create a file named `context-surf-handoff-[slug]-[timestamp].md` with the following structure:
...
## Completed Work (from Entire CLI session state)
[pull directly from CLI log — do not reconstruct from memory]

## In Progress at Exit (from Entire CLI session state)
[what the CLI shows as active at the moment of exit]
[include any partial outputs if useful]

## Pending Work (from plan-interview output — cross-reference Entire CLI to confirm what's genuinely not done)
...
> "Context wave is done. I've saved the session state to `[filename]`. The next session should load that file, run plan-interview to replan from [re-entry point], and catch the next wave. Here's what triggered the exit: [one sentence on drift signal]."
